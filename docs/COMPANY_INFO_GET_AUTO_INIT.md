...
After the user saves real data via **PUT** (or **POST /add** if the stub is still unconfigured), **`configured: true`** once `legal_name` is set.
...
GET /api/v1/company/info/get
  → 200, data.configured === false  → show setup form (empty)
  → 200, data.configured === true   → show/edit populated profile
...
| **Ambiguous state** | “Org opened settings once” ≠ “admin configured company”. Use **`configured`**, not row existence, for gating/onboarding. |
| **Analytics** | Count of profile rows overstates “configured companies”. Filter on `configured` or non-null `legal_name`. |
...
- **200 + `configured: false` without insert** — no DB side effect; frontend still handles empty state.
...
- **POST /add** after GET stub — allowed if stub is unconfigured (completes via update path); **400** if already configured.
