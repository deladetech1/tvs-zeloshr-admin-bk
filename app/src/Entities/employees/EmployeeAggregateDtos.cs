...
///
/// **Identifications:** Load types via <c>GET /api/v1/id-card-types/list</c> — use each item's <c>id_card_type_id</c> as
/// <c>identity.identifications[].id_type_id</c> on write (<c>id_number</c> + optional issue/expiry dates). One row per id card type per employee.
/// On read, each row also includes resolved <c>id_type: { id, name }</c> (same UUID as <c>id_card_type_id</c>).
///
...
public sealed class UpdateEmployeeAggregateRequest
{
    ...
    /// <c>true</c> + <c>identity.identifications[]</c> (including <c>[]</c>): replace section; unlisted rows deleted.
    /// </summary>
...
public sealed class EmployeeAggregateIdentityDto
{
    ...
    /// <summary>
    /// Government / company ID documents. <c>id_type_id</c> = <c>id_card_type_id</c> from <c>GET /api/v1/id-card-types/list</c>
    /// (or <c>GET /api/v1/id-card-types/get?id_card_type_id=</c>). On update include row <c>id</c> from GET to update; omit to add.
    /// </summary>
...
public sealed class EmployeeAggregateIdentityReadDto
{
    ...
    /// <summary>ID documents linked to <c>id-card-types</c>. Each item includes resolved <c>id_type</c> metadata on read.</summary>
