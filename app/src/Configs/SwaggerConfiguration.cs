            ...
            options.SwaggerDoc("v1", new OpenApiInfo
            {
                ...
                Description = $"""
                    ...
                    **Employees** · **Users** · **Currencies** · **Countries** · **Custom Fields** · **File Management** · **Organisation** (org chart, departments, branches) · **Company Settings** (employment types, ID card types, company info & offices, localization) · **Lifecycle Events** · **Audit Logs** · **Leave**
                    ...
                    1. **Get** — `GET /api/v1/company/info/get` (always 200; auto-creates stub when missing — `legal_name` seeded from Trovesuite `bus-id` / `cp_businesses.bus_name`; other fields null, `offices: []`)
                    ...
                    **Employee activation (no auth):** When HR finalises an employee, an activation email is sent with a link to `{{subdomain}}.{{EmployeePortalDomain}}/activate/{{token}}` (default TTL: `EmployeeActivationExpiryDays`, 7 days).
