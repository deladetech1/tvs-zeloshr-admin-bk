    ...
    private static void ApplyTypeLevelExample(OpenApiSchema schema, SchemaFilterContext context)
    {
        ...
        schema.Example = context.Type.Name switch
        {
            ...
            nameof(LeaveDashboardOnLeaveItemDto) => SwaggerExamples.LeaveDashboardOnLeaveItemData(
                SwaggerExamples.SampleLeaveRequestId.ToString(),
                "Ama Asante",
                "Annual Leave",
                "2026-09-25"),
            nameof(LeaveDashboardPendingItemDto) => SwaggerExamples.LeaveDashboardPendingItemData(
                SwaggerExamples.SampleLeaveRequestId.ToString(),
                "Kwame Asare",
                "Annual Leave",
                5,
                waiting: 53),
            nameof(LeaveDashboardLeavingItemDto) => SwaggerExamples.LeaveDashboardLeavingItemData(
            ...
            nameof(EmployeePasswordResetValidateDto) => SwaggerExamples.EmployeePasswordResetValidateData(),
            nameof(EmployeePasswordResetSetPasswordResultDto) => SwaggerExamples.EmployeePasswordResetSetPasswordData(),
            nameof(EmployeePasswordResetRequestResultDto) => SwaggerExamples.EmployeePasswordResetRequestData(),
            nameof(EmployeePasswordResetSetPasswordDto) => SwaggerExamples.EmployeePasswordResetSetPasswordBody(),
            nameof(EmployeePasswordResetRequestDto) => SwaggerExamples.EmployeePasswordResetRequestBody(),
        ...
        schema.Description = context.Type.Name switch
        {
            ...
            nameof(LeaveDashboardDto) => AppendDescription(schema.Description,
                "Leave Management home (`GET /leave/summary`): summary KPIs · flat employee lists for on_leave_today · pending_approvals · leaving_this_week."),
            nameof(LeaveDashboardSummaryDto) => AppendDescription(schema.Description,
                "KPI cards: on_leave_today · pending_approvals · leaving_this_week · low_balance_alert."),
            nameof(LeaveDashboardOnLeaveItemDto) => AppendDescription(schema.Description,
                "On leave today row: employee_name · profile_url · leave_type · returns_on · standard audit fields."),
            nameof(LeaveDashboardPendingItemDto) => AppendDescription(schema.Description,
            ...
            nameof(LeaveRequestListItemDto) => AppendDescription(schema.Description,
                "Admin request list row: nested employee · leave_type · start_date/end_date · days_requested · waiting_hours · audit fields."),
