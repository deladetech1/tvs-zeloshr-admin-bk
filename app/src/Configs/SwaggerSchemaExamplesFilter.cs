    ...
    private static void ApplyTypeLevelExample(OpenApiSchema schema, SchemaFilterContext context)
    {
        ...
        schema.Description = context.Type.Name switch
        {
            ...
            nameof(LeaveListDto) => AppendDescription(schema.Description,
                "Admin list payload: summary · requests[] (with remaining_days). Balances via GET /leave/balances/list."),
            nameof(LeaveMyRequestListDto) => AppendDescription(schema.Description,
                "My Leave requests only — no org summary or balances."),
            nameof(LeaveRequestListItemDto) => AppendDescription(schema.Description,
                $"Single request row. status: {SwaggerExampleHints.LeaveRequestStatus}. leave_type_id references GET /leave/types/list. approver_id is platform user id. remaining_days null when no balance row."),
            nameof(LeaveBalanceListItemDto) => AppendDescription(schema.Description,
                "Entitlement snapshot: entitled_days · used_days · remaining_days (entitled − used). leave_type_id references configured types."),
