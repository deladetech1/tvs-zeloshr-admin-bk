    ...
    private static void ApplyTypeLevelExample(OpenApiSchema schema, SchemaFilterContext context)
    {
        ...
        schema.Description = context.Type.Name switch
        {
            ...
            nameof(LeaveApprovalListDto) => AppendDescription(schema.Description,
                $"Leave Approvals table (`GET /leave/approvals/list`): pending_count · flat items[] — employee_name · title · profile_url · leave_type · leave_from/leave_to · leave_days · waiting · approved_by[] (full names). Tab: {SwaggerExampleHints.LeaveApprovalListTab}."),
            nameof(LeaveApprovalListItemDto) => AppendDescription(schema.Description,
                "Flat approvals row — no nested employee/leave_type objects. approved_by is a string array (LM → HOD)."),
