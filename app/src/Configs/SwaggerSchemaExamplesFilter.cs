    ...
    private static void ApplyTypeLevelExample(OpenApiSchema schema, SchemaFilterContext context)
    {
        ...
        schema.Example = context.Type.Name switch
        {
            ...
            nameof(LeaveApprovalStepDto) => new JsonObject
            {
                ["stage"] = "line_manager",
                ["status"] = "approved",
                ["approver"] = SwaggerExamples.LeaveApproverRefData(),
        ...
        schema.Description = context.Type.Name switch
        {
            ...
            nameof(LeaveDashboardDto) => AppendDescription(schema.Description,
                "Leave Management home widgets with enriched request rows."),
            nameof(LeaveMySummaryDto) => AppendDescription(schema.Description,
                "My Leave header: total_remaining_days · pending_requests · approved_this_year · balances[] (each with nested employee + leave_type)."),
            ...
            nameof(LeaveRequestListItemDto) => AppendDescription(schema.Description,
                $"List row with nested employee/job_title and leave_type. approval_stage: {SwaggerExampleHints.LeaveApprovalStage}. waiting_hours when pending."),
            nameof(LeaveRequestDetailDto) => AppendDescription(schema.Description,
                "Detail modal: nested employee · leave_type · working_days · public_holidays_in_range · balance_impact · approval_trail."),
            nameof(LeaveEmployeeRefDto) => AppendDescription(schema.Description,
                "Nested employee on list/detail rows. profile_url: DocumentReadDto (presigned_url ~24h) or null — same as GET /employees/list."),
            nameof(LeaveTypeRefDto) => AppendDescription(schema.Description,
                "Nested leave type: leave_type_id · name."),
            nameof(LeaveApproverRefDto) => AppendDescription(schema.Description,
                "Platform user who acted on a workflow step: user_id · full_name."),
            ...
            nameof(LeaveBalanceListItemDto) => AppendDescription(schema.Description,
                "Entitlement snapshot. Display via nested employee.full_name and leave_type.name — IDs are for filters/links only."),
