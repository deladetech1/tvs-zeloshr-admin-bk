    ...
    private static void ApplyTypeLevelExample(OpenApiSchema schema, SchemaFilterContext context)
    {
        ...
        schema.Description = context.Type.Name switch
        {
            ...
            nameof(LeaveDashboardDto) => AppendDescription(schema.Description,
                "Leave Management home: summary KPIs · on_leave_today[] (returns_on) · pending_approvals[] (all pending, oldest first; waiting_hours or days_since_last_approval) · leaving_this_week[] (start_date)."),
            nameof(LeaveMySummaryDto) => AppendDescription(schema.Description,
                "Leave Management landing page: summary KPIs · on_leave_today[] · pending_approvals[] · leaving_this_week[] · my (personal balances for logged-in employee)."),
            nameof(LeavePersonalSummaryDto) => AppendDescription(schema.Description,
                "Logged-in employee slice on GET /leave/summary: total_remaining_days · pending_requests · approved_this_year · balances[]."),
